...
val cols = SMSCorruptDF.columns.toSet
val SMSCount = SMSCorruptDF.cache().count()

dbTest("ET1-S-06-01-01", true, cols.contains("SMS"))
dbTest("ET1-S-06-01-02", true, cols.contains("SMSCorrupt"))
dbTest("ET1-S-06-01-03", 8, SMSCount)
...
val corruptPath = s"$workingDir/corruptSMS"
...
val testPath = s"$workingDir/corruptSMS/*/*/*"
val corruptCount = spark.read.json(testPath).count

dbTest("ET1-S-06-02-01", true, corruptCount >= 8)
