...
val dfMin = peopleDF.select(min("id")).first()(0).asInstanceOf[Int]
val dfMax = peopleDF.select(max("id")).first()(0).asInstanceOf[Int]
...
dbTest("ET1-S-04-01-01", 1, dfMin)
dbTest("ET1-S-04-01-02", 1000000, dfMax.asInstanceOf[Long])
...
// MAGIC 
// MAGIC <a href="https://docs.databricks.com/spark/latest/data-sources/sql-databases.html#managing-parallelism" target="_blank">Referencing the docs,</a> define the connection parameters for this read.
// MAGIC 
// MAGIC Use 8 partitions.
// MAGIC 
// MAGIC Assign the results to `peopleDFParallel`.
// MAGIC 
// MAGIC <img alt="Side Note" title="Side Note" style="vertical-align: text-bottom; position: relative; height:1.75em; top:0.05em; transform:rotate(15deg)" src="https://files.training.databricks.com/static/images/icon-note.webp"/> Setting the column for your parallel read introduces unexpected behavior due to a bug in Spark.  To make sure that Spark uses the capitalization of your column, use `""""id""""` for your column. <a href="https://github.com/apache/spark/pull/20370#issuecomment-359958843" target="_blank"> Monitor the issue here.</a>

// COMMAND ----------

// ANSWER
val peopleDFParallel = spark.read.jdbc(
...
// TEST - Run this cell to test your solution
dbTest("ET1-S-04-02-01", 8, peopleDFParallel.rdd.getNumPartitions)
...
println("Partitions: " + peopleDF.rdd.getNumPartitions)
println("Partitions: " + peopleDFParallel.rdd.getNumPartitions)

// COMMAND ----------

// MAGIC %md-sandbox
// MAGIC Use the `timeIt` function defined below and call `.describe()` on both your serial and parallel JDBC reads.  
// MAGIC 
// MAGIC <img alt="Side Note" title="Side Note" style="vertical-align: text-bottom; position: relative; height:1.75em; top:0.05em; transform:rotate(15deg)" src="https://files.training.databricks.com/static/images/icon-note.webp"/> `.describe()` computes summary statistics on a DataFrame
...
val time1 = timeIt(peopleDF.describe())
val time2 = timeIt(peopleDFParallel.describe())
